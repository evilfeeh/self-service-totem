...
describe('Customer Entity', () => {
  ...
  it("Should be able to validate email", () => {
    expect(() => new Email("invalidemail.com")).toThrow(InvalidEmailException);
    expect(() => new Email("invalidemail@")).toThrow(InvalidEmailException);
    expect(() => new Email("invalidemail@example")).toThrow(InvalidEmailException);
  ...
  it('should save customer', async () => {
    ...
    await expect(customer.findByCpf('123.456.789-09')).resolves.not.toThrow();
  }); 

  it('should throw CpfNotFoundException when cpf is not found', async () => {   
    await expect(customer.findByCpf('123.456.789-09')).rejects.toThrow(CpfNotFoundException);
  });
